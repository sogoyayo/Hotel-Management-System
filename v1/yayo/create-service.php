...
if (isset($_REQUEST['apptoken'])) {
	...
	if (CheckToken($mysqli, $apptoken)==true) {
		...
		if (($usertoken=='') or ($service_name=='') or ($date_from=='') or ($date_to=='') or ($location=='') or ($country=='') or ($city=='') or ($longitude=='') or ($latitude=='') or ($description=='') or ($social_media_link=='') or ($youtube_link=='') or ($price_adult=='') or ($price_child=='') or ($child_age_from=='') or ($child_age_to=='') or ($discounts=='') or ($discount_from=='') or ($discount_to=='') or ($min_pax_discount=='')) {
			// code...
			$array = [
				'success' => false,
					...
					if ($_FILES['avatar']) {
						...
						if ($uploadOk == 0) {
							...
							if (move_uploaded_file($_FILES["avatar"]["tmp_name"], $target_file)) {
								...
								if (createService($mysqli, $usertoken, $service_name, $date_from, $date_to, $location, $country, $city, $longitude, $latitude, $description, $social_media_link, $filename, $fileURL, $youtube_link, $price_adult, $price_child, $child_age_from, $child_age_to, $discounts, $discount_from, $discount_to, $min_pax_discount, $timestamp)==true) {
									// code...
									$array = [
										'success' => true,
