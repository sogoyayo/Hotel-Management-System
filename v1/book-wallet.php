...
$log = "Payment recieved for booking on $hotelname on $time";
		...
		if ($source == "Ezee") {
			// code...
			$ezeebooking = sendBookingToEzee($mysqli, $db, $data, $data->hoteltoken, EZEE_AUTH_KEY, EZEE_USERNAME);
			if($ezeebooking === true){
...
$array = array(
			'success' => false,
			'message' => "Payment made, failed to update your booking.."
		);
		$return = json_encode($array);
		echo "$return";
		exit();
	...
	$array = array(
			'success' => false,
			'message' => "Payment made, could not update your booking..",
			'output' => $ezeebooking
		);
		$return = json_encode($array);
		echo "$return";
