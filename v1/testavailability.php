...
$timestamp = input_check($_REQUEST['timestamp']);
...
// $sql = "SELECT * FROM contracts WHERE token='$token' and start_date > $timestamp or exp_date < $timestamp or start_date = $timestamp or exp_date = $timestamp and room_category != '0' and price_sgl !='undefined' and display = 1 and roomtoken='$roomtoken' ORDER BY id DESC LIMIT 1";
...
'real_end_date' => date("d-m-Y", $col['expiry_date'])
    ];
    ...
    if ($rownum > $count) {
        ...
        echo "$array";
            ...
            $array = [
            'success' => false,
            'message' => "No data, $timestamp (".date("d-m-Y", $timestamp).") - btme - $btime (".date("d-m-Y", $btime)."), atime - $atime (".date("d-m-Y", $atime)."), - $twentyfourhours (".date("d-m-Y", $twentyfourhours).") - 1645657200 (".date("d-m-Y", '1645657200').") - $roomtoken, $token"
        ];
